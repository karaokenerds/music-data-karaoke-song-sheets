...
def get_youtube_channel_id(google_token):
    print(f"Finding youtube username for authenticated user")
...
def get_liked_videos(userid, google_token):
    ...
    if 'items' in channels_response and len(channels_response['items']) > 0:
        print(channels_response['items'][0]['contentDetails'])
...
def identify_songs_from_youtube_videos(userid, liked_videos):
    ...
    print(f"Attempting to identify songs from {len(liked_videos)} youtube videos")
    liked_songs = []
    ...
    for video in liked_videos:
        ...
        if count == 0 or (count % 10) == 0:
            print(f"Inside youtube song identification loop, processed: {count} of total: {total}, identified: {identified_count}")
    ...
    print(f"Successfully identified {len(liked_songs)} songs from youtube videos")
